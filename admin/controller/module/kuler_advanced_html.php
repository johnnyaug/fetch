...
class ControllerModuleKulerAdvancedHtml extends Controller {
	...
	public function index() {
		...
		$kuler_advanced_html = $this->model_setting_setting->getSetting('kuler_advanced_html', $this->data['selected_store_id']);
		
		if (isset($this->request->post['kuler_advanced_html_module'])) {
			$this->data['modules'] = $this->request->post['kuler_advanced_html_module'];
		} else {
            if ($kuler_advanced_html)
            {
                $modules = array();

                if (isset($kuler_advanced_html['kuler_advanced_html_module']))
                {
                    $this->data['modules'] = $kuler_advanced_html['kuler_advanced_html_module'];
                }
            }
        }
		...
		if (isset($this->request->post['status'])) {
			...
			if ($kuler_advanced_html && isset($kuler_advanced_html['kuler_advanced_html_status'])) {
				$this->data['status'] = $kuler_advanced_html['kuler_advanced_html_status'];
			}
		}
	...
	protected function saveAction() {
		...
		$this->model_setting_setting->editSetting('kuler_advanced_html', array(
			'kuler_advanced_html_status' => $this->request->post['status'],
            'kuler_advanced_html_module' => !empty($this->request->post['kuler_advanced_html_module']) ? $this->request->post['kuler_advanced_html_module'] : array()
        ...
        if(isset($this->request->post['op']) && $this->request->post['op'] == 'close') {
            ...
            $this->response->redirect($this->url->link('module/kuler_advanced_html', 'token=' . $this->session->data['token'] . '&store_id=' . $this->request->post['store_id'], 'SSL'));
        }
	}
